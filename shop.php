...
if (!$response->status) {
    echo json_encode(['status' => false, 'message' => 'API returned an error: ' . $response->message]);
    exit();
}
...
if (!$brandresponse->status) {
    echo json_encode(['status' => false, 'message' => 'API returned an error: ' . $brandresponse->message]);
    exit();
}
        ...
        productItem.innerHTML = `
            <div class="card h-100">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="card-text fw-bold mb-0" style="color:#04048C;">#${parseFloat(product.product_price).toLocaleString()}</h5>
